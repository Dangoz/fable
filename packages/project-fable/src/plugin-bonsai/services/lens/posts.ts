  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      if (result.isErr()) {
        ...
        const backoffMs = Math.min(2 ** attempt * 1000 + Math.random() * 1000, MAX_BACKOFF_MS);
...
export const fetchPostById = async (_postId: string) => {
  try {
    const result = await withRetry<AnyPost | null>(() =>
      fetchPost(client, {
        post: postId(_postId),
      })
    );
...
export const fetchPostsBy = async (authorId: string, cursor?: Cursor | null) => {
  return await withRetry<Paginated<AnyPost>>(() =>
    fetchPosts(client, {
      filter: {
...
export const fetchAllCommentsFor = async (_postId: string): Promise<Post[]> => {
  ...
  do {
    const result = await withRetry<Paginated<AnyPost>>(() =>
...
export const fetchAllCollectorsFor = async (
  _postId: string,
  sorted = false
): Promise<`0x${string}`[]> => {
  ...
  do {
    const result = await withRetry<
      Paginated<{ account: { address: `0x${string}` }; firstAt: string }>
    >(() =>
      fetchWhoExecutedActionOnPost(client, {
        post: postId(_postId),
      })
    );
    ...
    allCollectors = [
      ...allCollectors,
      ...result.items.map((a) => ({ account: a.account.address, ts: a.firstAt })),
    ];
    nextPage = result.pageInfo.next;
  } while (nextPage);
...
export const fetchAllUpvotersFor = async (_postId: string): Promise<`0x${string}`[]> => {
  ...
  do {
    const result = await withRetry<Paginated<{ account: { address: `0x${string}` } }>>(() =>
      fetchPostReactions(client, {
        post: postId(_postId),
        filter: {
