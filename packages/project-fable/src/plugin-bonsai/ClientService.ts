...
import {
  BONSAI_CLIENT_VERSION,
  DEFAULT_FREEZE_TIME,
  FREE_GENERATIONS_PER_HOUR,
  PREMIUM_TEMPLATES,
...
class BonsaiClient {
  ...
  constructor() {
     ...
     */
    this.app.get('/metadata', async (_: express.Request, res: express.Response) => {
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        if (PREMIUM_TEMPLATES.includes(templateName)) {
          if (!(await canUpdate(creator, templateName))) {
            res
              .status(403)
              .json({ error: `not enough credits to generate preview for: ${templateName}` });
            return;
          }
        }
        ...
        this.handlePreviewCredits(
          template.clientMetadata.name,
          creator,
          response?.totalUsage,
          template.clientMetadata.defaultModel
        );
        ...
        if (agentId) {
          ...
          if (!(await canUpdate(creator, params.templateName))) {
            res
              .status(403)
              .json({
                error: `not enough credits to generate preview for: ${params.templateName}`,
              });
            return;
          }

          // generate the first page
          const response = await template.handler(
     ...
     */
    this.app.get('/post/:postId', async (req: express.Request, res: express.Response) => {
    ...
    this.app.post(
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        await this.mongo.media?.updateOne(
          { postId },
          { $set: { status: SmartMediaStatus.DISABLED } }
        );
     ...
     */
    this.app.get('/post/:postId/canvas', async (req: express.Request, res: express.Response) => {
      const { postId } = req.params;
    ...
    this.app.use(
      '/images',
      express.static(path.join(process.cwd(), 'temp', 'images'), {
        setHeaders: (res) => {
  ...
  private async handlePostUpdate(postId: string, forceUpdate?: boolean): Promise<void> {
    ...
    const response = await template?.handler(runtime as IAgentRuntime, data, undefined, {
      forceUpdate,
    });
    ...
    if (!response) {
      ...
      if (Math.floor(Date.now() / 1000) - data.updatedAt > DEFAULT_FREEZE_TIME) {
        ...
        await this.mongo.media?.updateOne(
          { postId },
          { $set: { status: SmartMediaStatus.FAILED } }
        );
        await this.removePostFromCache(data);
      }
      return;
    } else if (
      !response.refreshMetadata &&
      response.totalUsage.promptTokens === 0 &&
      response.totalUsage.imagesCreated === 0 &&
      response.totalUsage.videosCreated === 0
    ) {
    ...
    const needsStatusUpdate =
      data.status === SmartMediaStatus.DISABLED || data.status === SmartMediaStatus.FAILED;
    ...
    if (response.metadata || response?.refreshMetadata) {
      ...
      elizaLogger.info(
        `submitted lens refresh metadata request for post: ${postId} (${jobId} => ${status})`
      );
      if (status === 'FAILED') {
   ...
   * @returns {Promise<void>}
   */
  private async handlePreviewCredits(
  ...
  public start(port: number) {
    this.server.listen(port, () => {
      console.log(`BonsaiClient server running on http://localhost:${port}/`);
    });
  }
}
