...
function calculateTokenCost(tokensUsed: number, costPerMillionTokens: number): number {
...
export const canUpdate = async (address: string, template: string) => {
  ...
  return (
    credit?.creditsRemaining && credit.creditsRemaining >= (minCreditsForUpdate[template] ?? 0)
  );
};
...
export const decrementCredits = async (
  ...
  try {
    ...
    cost =
      calculateTokenCost(tokens.input, modelCosts['gpt-4o'].input) +
