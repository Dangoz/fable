...
export const parseAndUploadBase64Image = async (imageResponse) => {
  if (imageResponse.success && imageResponse.data?.[0]) {
    ...
    const base64Data = imageResponse.data[0].replace(/^data:image\/\w+;base64,/, '');
...
export const cacheImageStorj = async ({
  id,
  buffer,
  bucket = 'publication-history-media',
}: StorjUploadParams): Promise<StorjUploadResult> => {
  if (
...
export const cacheJsonStorj = async ({
  id,
  data,
  bucket = 'publication-history-metadata',
}: {
  id: string;
  data: any;
  bucket?: string;
