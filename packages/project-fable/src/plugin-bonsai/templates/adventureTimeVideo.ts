...
import {
  generateVideoLuma,
  DEFAULT_MODEL_ID as DEFAULT_MODEL_ID_LUMA,
  DEFAULT_DURATION_S,
} from '../services/luma';
    ...
    try {
      if (refresh) {
        ...
        totalUsage.customTokens[
          getModelSettings(ModelProviderName.OPENAI, ModelClass.SMALL)?.name
        ] = usage;
      ...
      totalUsage.customTokens[
        getModelSettings(ModelProviderName.VENICE, ModelClass.UNCENSORED)?.name
      ] = usage;
...
`;
      ...
      totalUsage.customTokens[getModelSettings(ModelProviderName.VENICE, ModelClass.MEDIUM)?.name] =
        videoPromptUsage;
      ...
      if (refresh) {
        ...
        const storjResult = await cacheVideoStorj({
          id: uuidv4(),
          buffer: await uriToBuffer(videoUri),
        });
  ...
  clientMetadata: {
    ...
    description:
      'The creator sets the stage for an evolving choose-your-own-adventure with video. Collectors & token holders decide the direction of the story.',
    image:
      'https://link.storjshare.io/raw/jv353tymzdmqupit7mkcjpjfmtpq/bonsai/adventureTimeVideo.png',
    options: {
      allowPreview: true,
