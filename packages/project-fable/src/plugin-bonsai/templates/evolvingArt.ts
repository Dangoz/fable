...
const evolvingArt = {
    ...
    try {
      ...
      if (isMediaStale(media as SmartMedia) || options?.forceUpdate) {
        ...
        if (comments.length < threshold) {
          elizaLogger.info(
            `evolvingArt:: post ${media?.postId} is stale but has not met comment threshold; skipping`
          );
      ...
      while (attempts < MAX_ATTEMPTS) {
        ...
        if (firstAttempt)
          elizaLogger.info('generating image with inpaint, context: ', imageUrl, imagePrompt);
  ...
  clientMetadata: {
    ...
    description:
      'Collect the post, buy tokens, and interact with the post (replies, upvotes) to evolve the image.',
    ...
    templateData: {
      form: z.object({
        style: z
          .string()
          .describe(
            'Define the style to maintain for all image generations - e.g. bright, neon green. [placeholder: Bright, neon green]'
          ),
        modelId: z
          .string()
          .nullish()
